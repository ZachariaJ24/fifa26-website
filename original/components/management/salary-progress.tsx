...
export function SalaryProgress({ current, max, projected }: SalaryProgressProps) {
  ...
  const isOverCap = projected ? projected > max : current > max
    ...
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        ...
        {projected && projected !== current && (
          <span className={`${isOverCap ? "text-red-400" : "text-green-400"}`}>
      ...
      <div className="relative">
        ...
        {projected && projected !== current && (
          <div
            className={`absolute top-0 h-2 border-r-2 ${isOverCap ? "border-red-500" : "border-green-300"}`}
