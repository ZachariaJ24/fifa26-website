  ...
  useEffect(() => {
    async function fetchTeamData() {
      try {
        ...
        // Fetch team details
        const { data: teamData, error: teamError } = await supabase.from("clubs").select("*").eq("id", teamId).single()
        ...
        const { data: eaStatsData, error: eaStatsError } = await supabase
          ...
          .select(
            "player_name, goals, assists, position, club_id, saves, goals_against, glsaves, glga, glshots, save_pct, glsavepct",
          )
          .in("club_id", [teamId])
