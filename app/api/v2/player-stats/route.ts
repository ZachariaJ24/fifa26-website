...
export async function GET() {
  ...
  try {
    ...
    if (matchStatsError) {
      ...
      throw new Error(`Error fetching match stats: ${matchStatsError.message}`)
    }

    console.log(`Fetched ${matchStats?.length || 0} match stats`)
    ...
    for (const stat of matchStats) {
        // Skip if stat doesn't have required fields
        if (!stat.player_id || !stat.match_id) {
            console.warn('Skipping stat with missing player_id or match_id:', stat);
            continue;
        }
        ...
        if (player) {
            ...
            player.passes_completed += stat.pass_complete || 0;
            player.passes_attempted += stat.pass_attempts || 0;
            player.tackles += stat.tackles || 0; // Using tackles field
            player.interceptions += stat.interceptions || 0;
            player.dribbles += stat.dribbles || 0; // Using dribbles field
            player.saves += stat.saves || 0;
            player.goals_against += stat.goals_against || 0;

            if ((stat.goals_against || 0) === 0 && player.position === 'GK') {
                player.clean_sheets! += 1;
            }
        } else {
            console.warn(`Player not found for stat: ${stat.player_id}`);
        }
    }
    ...
    console.log(`Returning ${finalStats.length} player stats`)
