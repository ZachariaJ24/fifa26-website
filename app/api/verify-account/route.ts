...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `Account verification attempt for ${email || "unknown email"} with token: ${token ? "provided" : "not provided"}`,
    )
    ...
    if (userWasVerified && result?.user) {
      try {
        console.log(`User ${result.user.email} account verified successfully, triggering automatic refresh...`)
  ...
  try {
    ...
    if (!publicUser) {
      ...
      const { error: createUserError } = await supabaseAdmin.from("users").insert({
        ...
        primary_position: metadata.primary_position || "CM",
