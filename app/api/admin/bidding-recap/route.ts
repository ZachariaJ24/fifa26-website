...
export async function GET() {
  try {
    ...
    // Get all bids from the transfers table (assuming bidding was migrated to transfers)
    const { data: bids, error: bidsError } = await supabase
      ...
      .order("transfer_fee", { ascending: false })
    ...
    const teamIds = [...new Set([
      ...bids.map((bid) => bid.from_team_id).filter(Boolean),
      ...bids.map((bid) => bid.to_team_id).filter(Boolean)
    ])]
    ...
    // Get teams data (using clubs table)
    ...
    bids.forEach((bid) => {
      const teamData = teamsMap.get(bid.to_team_id) // to_team_id is the bidding team
      const playerData = playersMap.get(bid.player_id)

      // Team stats
      if (!teamStatsMap.has(bid.to_team_id)) {
        teamStatsMap.set(bid.to_team_id, {
          team: {
            id: bid.to_team_id,
            name: teamData?.name || `Team_${bid.to_team_id}`,
      ...
      const teamStat = teamStatsMap.get(bid.to_team_id)
      ...
      if (bid.transfer_fee > playerBid.highestBid) {
        playerBid.highestBid = bid.transfer_fee
