    ...
    if (tableExists) {
      ...
      if (homeTeamEaClubId && eaMatchData.clubs && eaMatchData.clubs[homeTeamEaClubId]) {
        ...
        homePlayerStats = homePlayers.map((player: any) => {
          // Use centralized position mapping for consistency
          let position = "CM" // Default to Central Midfielder for soccer
          if (player.posSorted) {
            position = mapEaPositionToStandard(player.posSorted)
          } else if (player.position !== undefined && player.position !== null) {
            position = mapEaPositionToStandard(String(player.position))
          }

          const isPlayerGoalie = position === "GK"
