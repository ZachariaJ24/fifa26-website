  ...
  try {
    ...
    const fetchMatchesWithRetry = async (clubId: string, isHome: boolean) => {
      ...
      while (retryCount < maxRetries && matches.length === 0) {
        try {
          ...
          const endpoint = `https://proclubs.ea.com/api/fc/clubs/matches?matchType=${matchType}&platform=${platform}&clubIds=${clubId}`
    ...
    if (allMatches.length === 0) {
      try {
        ...
        const fallbackEndpoint = `https://proclubs.ea.com/api/fc/clubs/matches?platform=${platform}&matchType=${matchType}`
