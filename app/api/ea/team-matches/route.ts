...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      // Call EA Sports FC 26 API to get team matches with improved error handling
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 10000) // 10 second timeout

      const response = await fetch(
        `https://proclubs.ea.com/api/fc/clubs/matches?matchType=${matchType}&platform=${platform}&clubIds=${clubId}`,
        {
          headers: {
            "User-Agent":
