-- Migration: Convert Bidding System to Transfer Signing System
...
-- 3. Add system settings for transfer system
INSERT INTO system_settings (key, value) VALUES
('transfer_offers_enabled', 'true'),
('transfer_offer_duration', '14400'), -- 4 hours in seconds
('minimum_offer_increment', '2000000') -- $2M minimum increment
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;

-- 4. Create a view for active transfer offers
...
-- 5. Create a function to get the highest offer for each player
...
-- 6. Create a function to process expired offers
...
-- 7. Create a trigger to automatically update the updated_at timestamp
...
-- 8. Optional: Migrate existing bidding data (if you have any)
-- Uncomment this section if you want to migrate existing bids to transfer offers
/*
INSERT INTO player_transfer_offers (player_id, team_id, offer_amount, offer_expires_at, status)
SELECT 
    player_id, 
    team_id, 
    bid_amount, 
    bid_expires_at,
    CASE 
        WHEN bid_expires_at <= NOW() THEN 'expired'
        ELSE 'active'
    END
FROM player_bidding
WHERE bid_amount > 0;
...
-- Show the active transfer offers
