...
// Helper function to determine if a player is a goalkeeper
function isGoalie(position: string): boolean {
  return position === "GK" || position === "Goalkeeper" || position === "goalkeeper" || position === "0"
}
...
export function EaMatchStatistics({
  ...
  const fetchDirectlyFromEA = async (matchData: any) => {
    try {
      ...
      // Use the direct EA Sports FC 26 API endpoint format
      const eaApiUrl = `https://proclubs.ea.com/api/fc/clubs/matches?matchType=club_private&platform=common-gen5&clubIds=${homeClubId || awayClubId}`
        ...
        if (matchInfo.players) {
          ...
          Object.keys(matchInfo.players).forEach((clubId) => {
            ...
            Object.keys(clubPlayers).forEach((playerId) => {
              ...
              let position = "CM" // Default to Central Midfielder for soccer

              // Priority 1: Use posSorted field if available (most accurate)
              if (player.posSorted) {
                if (player.posSorted === "leftBack") position = "LB"
                else if (player.posSorted === "rightBack") position = "RB"
                else if (player.posSorted === "centerBack") position = "CB"
                else if (player.posSorted === "leftWing") position = "LW"
                else if (player.posSorted === "rightWing") position = "RW"
                else if (player.posSorted === "center") position = "CM"
                else if (player.posSorted === "goalkeeper") position = "GK"
                else position = mapEaPositionToStandard(player.posSorted)
              }
              // Priority 2: Use numeric position codes
              else if (player.position === "0") position = "GK"
              else if (player.position === "1") position = "RB"
              else if (player.position === "2") position = "LB"
              else if (player.position === "3") position = "RW"
              else if (player.position === "4") position = "LW"
              else if (player.position === "5") position = "CM"
