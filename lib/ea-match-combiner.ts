...
function determinePlayerPosition(player: any): string {
  ...
  if (player.posSorted) {
    return mapEaPositionToStandard(player.posSorted)
  }

  // Priority 2: Use numeric position codes (most reliable)
  if (player.position !== undefined && player.position !== null) {
    return mapEaPositionToStandard(String(player.position))
  }

  // Priority 3: Use skposition if available
  if (player.skposition) {
    return mapEaPositionToStandard(player.skposition)
  }

  // Priority 4: Check for goalie-specific stats to identify goalies
  if (player.glsaves !== undefined || player.saves !== undefined || player.goalsAgainst !== undefined) {
    return "GK"
  }

  // Priority 5: Use category if available
  if (player.category) {
    if (player.category === "goalkeeper") return "GK"
    if (player.category === "defense") return "CB" // Generic center back as fallback
    if (player.category === "midfield") return "CM" // Generic central midfielder as fallback
    if (player.category === "forward") return "ST" // Generic striker as fallback
  }

  // Priority 6: Look for position-specific stats to infer position
  if (player.skfow !== undefined || player.skfol !== undefined) {
    // Player has faceoff stats, likely a midfielder
    return "CM"
  }
  ...
  if (player.skgoals !== undefined || player.skassists !== undefined || player.skshots !== undefined) {
    return "ST" // Striker if we have offensive stats
  }
