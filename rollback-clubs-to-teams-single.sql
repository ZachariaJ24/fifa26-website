-- Rollback Script: Revert from MIGRATED state (clubs->teams, fixtures->matches)
-- This script assumes the migration was successful and tables are currently named clubs/fixtures
...
-- Foreign keys for matches table (only add if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'matches_home_team_id_fkey' AND table_name = 'matches') THEN
        ALTER TABLE matches ADD CONSTRAINT matches_home_team_id_fkey FOREIGN KEY (home_team_id) REFERENCES public.teams(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'matches_away_team_id_fkey' AND table_name = 'matches') THEN
        ALTER TABLE matches ADD CONSTRAINT matches_away_team_id_fkey FOREIGN KEY (away_team_id) REFERENCES public.teams(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'ea_player_stats_match_id_fkey' AND table_name = 'ea_player_stats') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'ea_team_stats_match_id_fkey' AND table_name = 'ea_team_stats') THEN
        ALTER TABLE ea_team_stats ADD CONSTRAINT ea_team_stats_match_id_fkey FOREIGN KEY (match_id) REFERENCES public.matches(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'game_availability_match_id_fkey' AND table_name = 'game_availability') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'lineups_match_id_fkey' AND table_name = 'lineups') THEN
        ALTER TABLE lineups ADD CONSTRAINT lineups_match_id_fkey FOREIGN KEY (match_id) REFERENCES public.matches(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'match_events_match_id_fkey' AND table_name = 'match_events') THEN
