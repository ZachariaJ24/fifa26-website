-- FIFA 26 League - Complete System Setup
...
-- Simple policies to avoid column reference issues
-- Division policies
CREATE POLICY "Allow authenticated users to read divisions" ON divisions
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all authenticated users to modify divisions" ON divisions
    FOR ALL USING (auth.role() = 'authenticated');

-- Transfer offer policies
CREATE POLICY "Allow authenticated users to read transfer offers" ON player_transfer_offers
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated users to modify transfer offers" ON player_transfer_offers
    FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Allow authenticated users to modify signings" ON player_signings
    FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Allow authenticated users to modify transfers" ON player_transfers
    FOR ALL USING (auth.role() = 'authenticated');
...
-- Function to get promotion/relegation status (simplified)
...
BEGIN
    ...
    WITH ranked_teams AS (
        SELECT 
            ...
            t.name,
            0 as points,
            ROW_NUMBER() OVER (PARTITION BY t.division ORDER BY t.name) as rank,
            COUNT(*) OVER (PARTITION BY t.division) as total_teams
        FROM teams t
        WHERE t.is_active = true AND t.division IS NOT NULL
    )
    SELECT 
        rt.division,
        rt.name,
...
-- Function to get transfer recap data
...
-- ============================================================================
-- 9. DIVISION STANDINGS VIEW
-- ============================================================================

-- Create a simple view without team_stats dependency
